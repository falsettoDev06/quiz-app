...
function QuizPlay() {
  ...
  useEffect(() => {
    if (ui.counter === 0 && !ui.reveal && !timer.isRunning) {
      timer.handleStart();
    }
    if (ui.counter === 9 && ui.reveal) {
      timer.handleStop();
    }
  }, [ui.counter, ui.reveal, timer.isRunning, timer.handleStart, timer.handleStop]);

  return (
    <div className="relative bg-base-200">
      <div className={ui.reveal && ui.counter === 9 ? "blur-sm" : ""}>
        <div className="flex flex-col justify-evenly items-center min-h-screen">
          <div className="flex py-10">
            ...
            <p className={handleCountdownColor()}>{timer.smallTimer}</p>
          </div>
          <QuizCard />
          <button
            className={`btn btn-accent btn-md lg:btn-lg w-80 h-15 my-2 lg:h-18 lg:w-100 rounded-full text-3xl shadow-xl transition-transform hover:scale-105 duration-100 ease-in-out ${!ui.reveal || ui.counter === 9 ? "invisible" : ""}`}
          ...
          <progress
            ...
            value={(ui.counter + 1) * 10}
            max="100"
          ></progress>
        </div>
      </div>
      {ui.reveal && ui.counter === 9 && (
        <button
